...
def test_list_files_windows_powershell() -> None:
    cmd = suggest_command("list files in this folder", os_name="Windows", shell="PowerShell")
    assert cmd == "Get-ChildItem"


def test_list_files_windows_cmd() -> None:
    cmd = suggest_command("list files", os_name="Windows", shell="CMD")
    assert cmd == "dir"
